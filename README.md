...
Klasė `žmogus` realizuota kaip abstrakti – ji turi grynai virtualius metodus
`vardas()` ir `pavarde()`. Dėl šios priežasties `žmogus` tipo objektų sukurti
negalima.

Bandymas sukurti `žmogus` objektą sukelia kompiliavimo klaidą,
todėl programoje kuriami tik išvestinės klasės `Studentas` objektai.